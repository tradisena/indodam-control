...
SETUP = {
    ...
    "sr": {"auto_entry_enabled": True, "near_pct": 0.10, "baseline": "ADR14", "candle_lookback": 15},
    ...
    try:
        if os.path.exists(PERSIST_FILE):
            with open(PERSIST_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            # Lakukan migrasi data sebelum update
            if 'click_xy' in data and isinstance(data['click_xy'], list):
                ...
                for i, item in enumerate(data['click_xy']):
                    if isinstance(item, dict) and ('title' not in item or 'func' not in item):
                        ...
                        item.update(new_item) # Update in-place to preserve other keys
        ...
        # Jika tidak ada akun yang terdaftar, tambahkan akun default dari ENV
...
def compute_sr_thresholds(sym):
    lookback = int(SETUP.get("sr", {}).get("candle_lookback", 15))
    ...
    near_pct_val = float(SETUP.get("sr", {}).get("near_pct", 10.0))
...
def sr_auto_trade(sym):
    ...
    # Cek jumlah open posisi "Auto SR" saat ini
    open_pos = positions(sym) or []
    auto_sr_positions = [p for p in open_pos if classify_open_exec(getattr(p, 'comment', '')) in ('Auto RS Buy', 'Auto RS Sell')]
    if len(auto_sr_positions) >= 4:
        return
    ...
    for side in ("buy", "sell"):
        ...
        if trig.get("pending") and open_count(sym) > 0:
            trig["pending"] = False
            print(f"[Auto RS {side.upper()}] Status 'pending' dihapus (posisi terdeteksi).", flush=True)
    ...
    # SELL at Resistance via Clicker
    if (sr_sell_setup and SETUP.get("sr_sell_enabled") and SR_TRIGGER["sell"]["armed"] and 
        (not SR_TRIGGER["sell"].get("pending")) and price >= thresholds["top"] and 
        (now - SR_TRIGGER["sell"]["last_ts"]) >= SR_MIN_GAP and
        current_candle_ts > SR_TRIGGER["sell"]["last_trigger_candle_ts"] and
        thresholds["top"] != SR_TRIGGER["sell"]["last_trigger_level"]):
        
        # --- LOGIKA PENGUNCI ARAH ---
        if STATE.get("arah_posisi_terkunci") and STATE.get("arah_posisi_terkunci") != "SELL":
            SR_TRIGGER["sell"]["last_ts"] = now # Update timestamp to prevent rapid checks
            return # Arah terkunci ke BUY, abaikan sinyal SELL

        x, y = sr_sell_setup.get("x"), sr_sell_setup.get("y")
        if x and y and x > 0 and y > 0:
            # Jika ini adalah trade pertama dalam siklus, kunci arahnya
            ...
            reason = "Auto RS SELL"
            print(f"[{reason}] Terpicu. Menyiapkan untuk membuka posisi. Harga {price:.2f} >= Top {thresholds['top']:.2f}", flush=True)
            ...
            SR_TRIGGER["sell"].update({
                "armed": False, 
                "pending": True, 
                "last_ts": now,
                "last_trigger_candle_ts": current_candle_ts,
                "last_trigger_level": thresholds["top"]
            })
        else:
            message = f"Auto RS SELL diabaikan. Koordinat tidak valid untuk 'auto_sr_sell'."
            print(f"[Auto RS SELL] {message}", flush=True)
            STATE["last_system_message"] = {"text": message, "type": "warn"}
            SR_TRIGGER["sell"]["last_ts"] = now # Prevent rapid invalid attempts

    # BUY at Support via Clicker
    elif (sr_buy_setup and SETUP.get("sr_buy_enabled") and SR_TRIGGER["buy"]["armed"] and 
          (not SR_TRIGGER["buy"].get("pending")) and price <= thresholds["bottom"] and 
          (now - SR_TRIGGER["buy"]["last_ts"]) >= SR_MIN_GAP and
          current_candle_ts > SR_TRIGGER["buy"]["last_trigger_candle_ts"] and
          thresholds["bottom"] != SR_TRIGGER["buy"]["last_trigger_level"]):

        # --- LOGIKA PENGUNCI ARAH ---
        if STATE.get("arah_posisi_terkunci") and STATE.get("arah_posisi_terkunci") != "BUY":
            SR_TRIGGER["buy"]["last_ts"] = now # Update timestamp to prevent rapid checks
            return # Arah terkunci ke SELL, abaikan sinyal BUY
        ...
        if x and y and x > 0 and y > 0:
            ...
            reason = "Auto RS BUY"
            print(f"[{reason}] Terpicu. Menyiapkan untuk membuka posisi. Harga {price:.2f} <= Bottom {thresholds['bottom']:.2f}", flush=True)
            initial_pos_count = _get_open_count(sym)
            STATE["pending_open"] = {"side": "BUY", "ts": 0, "retries": 0, "x": x, "y": y, "reason": reason, "initial_pos_count": initial_pos_count}
            SR_TRIGGER["buy"].update({
                "armed": False, 
                "pending": True, 
                "last_ts": now,
                "last_trigger_candle_ts": current_candle_ts,
                "last_trigger_level": thresholds["bottom"]
            })
        else:
            message = f"Auto RS BUY diabaikan. Koordinat tidak valid untuk 'auto_sr_buy'."
            print(f"[Auto RS BUY] {message}", flush=True)
            STATE["last_system_message"] = {"text": message, "type": "warn"}
            SR_TRIGGER["buy"]["last_ts"] = now # Prevent rapid invalid attempts
...
def manage_tpsm_tpsb_mode(sym):
    ...
    # Kondisi untuk mengaktifkan Auto TPSB: 4 atau lebih posisi terbuka
    if open_positions >= 4 and not SETUP.get("auto_tpsb_enabled"):
    ...
    # Kondisi untuk kembali ke Auto TPSM: kurang dari 4 posisi terbuka
    elif open_positions < 4 and not SETUP.get("tpsm_auto"):
    ...
    while not stop_flag.is_set():
        ...
        try:
            if now >= next_tick:
                ...
                # Manajemen state siklus (reset jika perlu)
                check_and_reset_trade_direction_lock(sym)
                # Manajemen mode TPSM/TPSB berdasarkan jumlah posisi live
                manage_tpsm_tpsb_mode(sym)

                # 1. Jalankan semua logika yang bisa membuat 'tugas' (pending_open/pending_close)
                sr_auto_trade(sym)
                auto_cross_trade(sym)
                auto_tpsb_tick(sym)
                auto_tpsm_tick(sym)
                trailing_stop_tick(sym)
                # 2. Jalankan eksekutor yang memproses 'tugas' tersebut
                retry_and_verify_open_tick(sym)
                retry_and_verify_close_tick(sym)
                # 3. Jalankan logika sesi lainnya
                try_break_event(sym)
                session_tick(sym)
                next_tick = now + 1.0
        except Exception as e:
...
def api_sr_toggle():
    SETUP["sr_buy_enabled"] = bool((request.get_json(force=True) or {}).get("on"))
    persist_save()
    return jsonify({"ok": True})
...
def api_setup_sr():
    ...
    if isinstance(data, dict):
        current_sr_settings = SETUP.get("sr", {})
        # Update only the keys provided, don't overwrite the whole object
        if "candle_lookback" in data: current_sr_settings["candle_lookback"] = int(data["candle_lookback"])
        if "near_pct" in data: current_sr_settings["near_pct"] = float(data["near_pct"])
        SETUP["sr"] = current_sr_settings
        persist_save()
        return jsonify({"ok": True, "saved": SETUP["sr"]})
